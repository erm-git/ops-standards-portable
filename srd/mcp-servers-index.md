...
This file is a quick, human/LLM-friendly index of MCP servers commonly used on this machine: what they are, where they live, and how they’re configured.
...
On this machine those variables are populated by `~/.config/mcp-secrets/load.sh` (sourced by `~/.bashrc` via `~/.bash_local`).
...
- `/opt/tools/rpg-srd/mcp_server.py` — local indexed SRD search/read for `/srv/dev/rpg-srd`
- `/opt/tools/rpg-srd/rpg_srd_text_etl_mcp_server.py` — rpg-srd ETL workflows
